...
vertex rotate(vertex v) {
    ...
    // Y rotation
    rotation_m << cos(M_PI/6), 0, sin(M_PI/6),
    0, 1, 0,
    -sin(M_PI/6), 0, cos(M_PI/6);
    ...
    Eigen::Vector3f v3(v.x, v.y, v.z);
    
    Eigen::Vector3f result = rotation_m * v3;
    return vertex{
        result[0], 
        result[1], 
        result[2]
...
int main (int argc, char* argv[]) {
    ...
    for (int i = 0; i < amongus.n_vertices; i++) {
        amongus.vertices[i] = rotate(amongus.vertices[i]);
        amongus.vertices[i] = perspective(amongus.vertices[i]);
    }
